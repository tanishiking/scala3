...
class MissingAnnotationTest:
  ...
  def ignoresUndocumentedAnnotationsMissingFromDocClasspath =
    ...
    try
      ...
      val barOutput = root.resolve("bar")
      compileStage(barOutput, Seq(fooOutput), source(root, "bar.scala"))
      ...
      Scaladoc.run(
        testArgs(
          Seq(
            fooOutput.resolve("foo/Foo.tasty").toFile,
            barOutput.resolve("bar/Bar.tasty").toFile,
          ),
          docOutput
        ).copy(
          classpath = Seq(fooOutput.toString, barOutput.toString, javaClasspath)
